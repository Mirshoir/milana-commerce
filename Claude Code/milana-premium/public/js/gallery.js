...
(() => {
  ...
  let allProducts = [];
  let currentCat = "";
  const DEFAULT_CATS = ["pajamas", "robes", "homewear", "loungewear"];
  ...
  function buildTabs() {
    ...
    const present = [...new Set([
      ...DEFAULT_CATS.filter((c) => allProducts.some((p) => p.category === c)),
      ...allProducts.map((p) => p.category).filter(Boolean),
    ])];
    tabsEl.innerHTML = ["", ...present].map((c) =>
      `<button type="button" data-lb-cat="${esc(c)}" class="${c === currentCat ? "is-on" : ""}">${esc(c === "" ? tr("shop.all") : cn(c))}</button>`).join("");
  }
