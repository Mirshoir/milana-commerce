...
(() => {
  ...
  async function handleRecover(form) {
    const data = formData(form);
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(String(data.email || ""))) throw new Error("email");
    if (provider === "firebase" && firebaseAuth && window.__milanaFirebase) {
      await window.__milanaFirebase.authMod.sendPasswordResetEmail(firebaseAuth, data.email);
  ...
  function wirePage() {
    ...
    page.querySelectorAll("[data-auth-form]").forEach((form) => {
      form.addEventListener("submit", async (e) => {
        ...
        try {
          let result;
          if (mode === "recover") result = await handleRecover(form);
          else if (provider === "firebase") await handleFirebase(form, mode);
          else await handleLocal(form, mode);
          setMsg(mode, result?.reset_sent ? t("auth.resetEmailSent") : t("auth.ready"), false);
        } catch (ex) {
  ...
  function syncRecoveryForm() {
    const usesFirebaseReset = provider === "firebase" && Boolean(firebaseAuth && window.__milanaFirebase);
    document.querySelectorAll("[data-local-recovery]").forEach((element) => {
      element.hidden = usesFirebaseReset;
    });
    const help = document.querySelector('[data-auth-form="recover"] .auth-help');
    if (help) {
      const key = usesFirebaseReset ? "auth.firebaseRecoverLead" : "auth.recoverLead";
  ...
  window.addEventListener("i18n:change", () => { renderLinks(); renderAccount(); syncRecoveryForm(); });
