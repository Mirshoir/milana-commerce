...
const BOOLEAN_COLUMNS = new Set([
  ...
  "catalog_product_overrides.preorder",
]);
const REQUIRED_PRODUCT_TEXT_COLUMNS = new Set(
  Object.entries(PRODUCT_RUNTIME_COLUMN_DEFINITIONS)
    .filter(([, definition]) => definition.startsWith("TEXT NOT NULL"))
    .map(([column]) => column),
);
...
function normalizeValue(table, column, value, row = {}) {
  ...
  if (table === "telegram_order_outbox" && column === "sent_at" && !value) return null;
  if (table === "products" && REQUIRED_PRODUCT_TEXT_COLUMNS.has(column) && value == null) return "";
...
async function upsertRows(client, table, rows, columns) {
  ...
  const conflictColumn = ["settings", "sessions", "customer_sessions", "phone_otps", "email_otps", "catalog_product_overrides", "telegram_order_outbox"].includes(table)
    ? columns[0] : "id";
...
async function main() {
  ...
  try {
    ...
    for (const table of TABLES) {
      ...
      if (table === "products") {
        const missingRequiredColumns = PRODUCT_REQUIRED_COLUMNS.filter((column) => !allowed.has(column));
        if (missingRequiredColumns.length) {
          throw new Error(
            `PostgreSQL products schema is missing required columns: ${missingRequiredColumns.join(", ")}`,
          );
        }
      }
