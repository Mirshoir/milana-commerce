...
const TYPE_LABELS = {
  ...
  uz: {
    ...
    capri: "kapri", shorts: "shortik", top: "mayka",
  },
};
...
function sourceText(product) {
  ...
  return [
    ...
    product?.material, product?.composition,
    fabric.en, fabric.ru, fabric.uz,
  ].filter(Boolean).join(" ").toLowerCase().replace(/ё/g, "е");
}

function isFabricPlaceholder(value) {
  return /(?:состав\s+не\s+указан|composition\s+not\s+specified|tarkibi\s+ko['’‘ʻ`]?rsatilmagan)/iu
    .test(String(value || ""));
}

function cleanFabricText(value) {
  const text = String(value || "").trim();
  return isFabricPlaceholder(text) ? "" : text;
}
...
function localizedMaterial(product, lang = "en") {
  ...
  /* Материал не распознан — показываем собственное описание модели из каталога.
     Пустое значение лучше служебной заглушки: карточка просто скроет строку. */
