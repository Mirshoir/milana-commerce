...
Трёхъязычный (UZ / RU / ENG) сайт премиального бренда домашней одежды:
...
FIREBASE_APPLE_ENABLED=1                # set to 1 after Apple provider is enabled in Firebase
```
...
Google. Чтобы включить кнопку входа через Apple, задайте `FIREBASE_APPLE_ENABLED=1`
после включения Apple provider в Firebase. В заказах сохраняется `customer_id`,
чтобы позже связать сайт, Flutter приложения и ERP (`Shmirzaev/Milana-ERP`) через
один профиль клиента.
