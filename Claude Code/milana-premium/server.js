...
const FIREBASE_APPLE_ENABLED = FIREBASE_ENABLED && envBool(process.env.FIREBASE_APPLE_ENABLED);
const FIREBASE_CERTS_URL = (
  process.env.FIREBASE_CERTS_URL
  || "https://www.googleapis.com/robot/v1/metadata/x509/securetoken@system.gserviceaccount.com"
).trim();
const GOOGLE_CLIENT_ID = (process.env.GOOGLE_CLIENT_ID || process.env.GOOGLE_OAUTH_CLIENT_ID || "").trim();
const GOOGLE_TOKENINFO_URL = (process.env.GOOGLE_TOKENINFO_URL || "https://oauth2.googleapis.com/tokeninfo").trim();
const GOOGLE_USERINFO_URL = (process.env.GOOGLE_USERINFO_URL || "https://www.googleapis.com/oauth2/v3/userinfo").trim();
...
const CATALOG_PANEL_IDS = [
  ...
  "nightgowns", "sets", "tshirts", "kids", "clothing_sets",
];
...
const postgresCatalog = POSTGRES_CATALOG_ENABLED
  ? new PostgresCatalog({
      ...
      engagementEnabled: POSTGRES_COMMERCE_ENABLED,
    })
  : null;
...
/* ============ справочники полей карточки товара ==========================
   заполняются в админке, товар выбирает значение из готового списка         */
const DEFAULT_CATEGORIES = ["pajamas", "robes", "homewear", "loungewear"];
const DICT_KINDS = [
...
function referenceValues(kind, defaults = []) {
  const configured = dictList(kind);
  return configured.length ? configured : [...defaults];
}

function referenceAllows(kind, value, defaults = []) {
  const clean = String(value ?? "").trim();
  return Boolean(clean) && referenceValues(kind, defaults).includes(clean);
}

function referenceValue(kind, value, defaults = [], max = 120) {
  const clean = str(value, max).trim();
  return referenceAllows(kind, clean, defaults) ? clean : "";
}
...
function dictUsage() {
  const usage = Object.fromEntries(DICT_KINDS.map((kind) => [kind, {}]));
...
function inferProductType(product = {}) {
  const rawDeclared = String(product.product_type || "").trim();
  const declared = rawDeclared.toLowerCase();
  if (PRODUCT_TYPE_IDS.includes(declared)) return declared;
  if (dictList("product_type").includes(rawDeclared)) return rawDeclared;
  ...
  const panelType = {
    ...
    nightgowns: "sarochka", sets: "set", clothing_sets: "set", tshirts: "tshirt",
...
(() => {
  ...
  if (!cols.includes("preorder")) db.exec("ALTER TABLE products ADD COLUMN preorder INTEGER DEFAULT 0");
  if (!cols.includes("copy_manual")) db.exec("ALTER TABLE products ADD COLUMN copy_manual INTEGER NOT NULL DEFAULT 0");
  const overrideCols = db.prepare("PRAGMA table_info(catalog_product_overrides)").all().map((c) => c.name);
  if (!overrideCols.includes("preorder")) {
    db.exec("ALTER TABLE catalog_product_overrides ADD COLUMN preorder INTEGER DEFAULT 0");
  }
...
(function seedDictionaryDefaults() {
  ...
  const knownCatalogPanels = referenceValues("catalog_panel", CATALOG_PANEL_IDS);
  const ungrouped = db.prepare(`
    SELECT id, model_no, variant, gender, category, catalog_panel, name, desc_en, desc_ru, desc_uz
    FROM products
    WHERE catalog_panel IS NULL OR catalog_panel='' OR catalog_panel NOT IN (${knownCatalogPanels.map(() => "?").join(",")})
  `).all(...knownCatalogPanels);
  if (ungrouped.length) {
  ...
  const knownProductTypes = referenceValues("product_type", PRODUCT_TYPE_IDS);
  const untypedProducts = db.prepare(`
    ...
    WHERE product_type IS NULL
       ...
       OR LOWER(TRIM(product_type)) NOT IN (${knownProductTypes.map(() => "?").join(",")})
  `).all(...knownProductTypes.map((value) => value.toLowerCase()));
...
/* Existing installations start with the four original values plus any
   categories already used by products. Afterwards the admin owns this list. */
if (!dictList("category").length) {
  const usedCategories = db.prepare(`
    SELECT DISTINCT TRIM(category) value
    FROM products
    WHERE TRIM(COALESCE(category,'')) <> ''
  `).all().map((row) => row.value);
  dictReplace("category", [...new Set([...DEFAULT_CATEGORIES, ...usedCategories])]);
}

/* Add the remaining Basic-field lists once. A migration marker prevents an
   intentionally emptied list from being repopulated on every restart. */
if (getSetting("basic_dictionaries_v1") !== "1") {
  const defaults = {
    gender: ["women", "men", "kids", "unisex"],
    category: DEFAULT_CATEGORIES,
    catalog_panel: CATALOG_PANEL_IDS,
    ...
    material: [],
    composition: [],
    season: ["Всесезонный", "Лето", "Демисезон", "Зима"],
    collection: ["ss26"],
  };
  db.exec("BEGIN");
  try {
    for (const kind of DICT_KINDS) {
      ...
      const used = col
        ? db.prepare(`SELECT DISTINCT TRIM(${col}) value FROM products WHERE TRIM(COALESCE(${col},'')) <> '' ORDER BY 1`).all().map((row) => row.value)
        : [];
      const existing = dictList(kind);
      dictReplace(kind, [...new Set([...existing, ...(defaults[kind] || []), ...used])]);
    }
    setSetting("basic_dictionaries_v1", "1");
    db.exec("COMMIT");
  } catch (error) {
    db.exec("ROLLBACK");
    throw error;
...
const SECURITY_HEADERS = {
  ...
  "Content-Security-Policy": [
    ...
    `connect-src 'self' ${CATALOG_ASSET_ORIGIN} https://identitytoolkit.googleapis.com https://securetoken.googleapis.com https://www.googleapis.com https://firestore.googleapis.com https://appleid.apple.com`,
    "base-uri 'self'",
    "form-action 'self'",
...
async function verifyGoogleAccessToken(accessToken) {
  ...
  const tokenInfo = await fetch(GOOGLE_TOKENINFO_URL + "?access_token=" + encodeURIComponent(token));
...
async function upsertCustomer({
  ...
  if (postgresCommerce) {
    return postgresCommerce.upsertCustomer({
      ...
      preserve_managed_fields: Boolean(preserve_managed_fields),
    });
  }
  ...
  if (existing) {
    db.prepare(`
      UPDATE customers SET
        ...
        account_type=CASE WHEN ? THEN account_type ELSE COALESCE(NULLIF(?,''), account_type) END,
        approval_status=CASE WHEN ? THEN approval_status ELSE COALESCE(NULLIF(?,''), approval_status) END,
        ...
        phone_verified=CASE
          WHEN NULLIF(?, '') IS NULL THEN phone_verified
          WHEN phone <> ? THEN ?
          ELSE MAX(phone_verified, ?)
        END,
        provider=?,
...
const SUPPORT_TOPICS = ["general", "catalog", "price", "delivery", "defect", "payment", "order", "account_deletion"];
...
function checkoutIdempotency(req, body, customer) {
  const key = String(req.headers["idempotency-key"] || "").trim();
  if (!key) return null;
  if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{7,127}$/.test(key)) throw new Error("idempotency_key");
  const scope = customer ? `customer:${customer.id}` : "guest";
  return {
    key,
    requestHash: sha256(`${scope}\n${stableJson(body)}`),
  };
}

function storedCheckoutResponse(record) {
...
function checkoutResponse({ orderId, number, amount, orderType, manager, paymentMethod }) {
  return {
    id: orderId,
    order_id: orderId,
    number,
...
const telegramOutboxNumber = (name, fallback, min, max) =>
  Math.max(min, Math.min(max, Number(process.env[name]) || fallback));
const TELEGRAM_OUTBOX_POLL_MS = telegramOutboxNumber("TELEGRAM_OUTBOX_POLL_MS", 5_000, 25, 300_000);
const TELEGRAM_OUTBOX_RETRY_BASE_MS = telegramOutboxNumber("TELEGRAM_OUTBOX_RETRY_BASE_MS", 30_000, 25, 3_600_000);
const TELEGRAM_OUTBOX_RETRY_MAX_MS = telegramOutboxNumber("TELEGRAM_OUTBOX_RETRY_MAX_MS", 3_600_000, 25, 86_400_000);
const TELEGRAM_OUTBOX_LEASE_MS = telegramOutboxNumber("TELEGRAM_OUTBOX_LEASE_MS", 120_000, 100, 3_600_000);
const TELEGRAM_OUTBOX_TIMEOUT_MS = telegramOutboxNumber("TELEGRAM_OUTBOX_TIMEOUT_MS", 10_000, 100, 60_000);
const TELEGRAM_OUTBOX_BATCH_SIZE = telegramOutboxNumber("TELEGRAM_OUTBOX_BATCH_SIZE", 10, 1, 50);
...
async function notifyTelegramOrder(order) {
  ...
  const timer = setTimeout(() => controller.abort(), TELEGRAM_OUTBOX_TIMEOUT_MS);
  try {
    ...
    if (!response.ok || result?.ok === false) {
      ...
      error.retryAfterMs = Math.max(0, Number(result?.parameters?.retry_after) || 0) * 1000;
      throw error;
    }
    return result;
  } finally {
...
function claimSqliteTelegramOutbox(lockToken, nowMs, staleBeforeMs) {
  ...
  try {
    ...
    const rows = db.prepare(`
      ...
      LIMIT ?
    `).all(nowMs, staleBeforeMs, TELEGRAM_OUTBOX_BATCH_SIZE);
    if (rows.length) {
      const placeholders = rows.map(() => "?").join(",");
      db.prepare(`
        UPDATE telegram_order_outbox
        SET status='sending', attempts=attempts+1, locked_at=?, lock_token=?,
            updated_at=datetime('now')
        WHERE order_id IN (${placeholders})
      `).run(nowMs, lockToken, ...rows.map((row) => row.order_id));
    }
    db.exec("COMMIT");
...
function markSqliteTelegramOutboxSent(row, messageId) {
  return db.prepare(`
    UPDATE telegram_order_outbox
    SET status='sent', message_id=?, sent_at=datetime('now'), updated_at=datetime('now'),
        locked_at=NULL, lock_token='', last_error=''
    WHERE order_id=? AND status='sending' AND lock_token=?
  `).run(String(messageId || ""), row.order_id, row.lock_token).changes;
}

function markSqliteTelegramOutboxRetry(row, error, nextAttemptAt) {
  return db.prepare(`
    UPDATE telegram_order_outbox
    SET status='retry', next_attempt_at=?, last_error=?, updated_at=datetime('now'),
        locked_at=NULL, lock_token=''
    WHERE order_id=? AND status='sending' AND lock_token=?
  `).run(
    nextAttemptAt,
    String(error?.message || error || "telegram_failed").slice(0, 500),
    row.order_id,
    row.lock_token,
  ).changes;
}
...
let telegramOutboxWakeRequested = false;
let telegramOutboxStopping = false;
let telegramOutboxActivePromise = Promise.resolve();

async function processTelegramOutbox() {
  if (!TELEGRAM_ORDERS_ENABLED || telegramOutboxStopping) return false;
  const nowMs = Date.now();
  const lockToken = crypto.randomUUID();
  const staleBeforeMs = nowMs - TELEGRAM_OUTBOX_LEASE_MS;
  const rows = postgresCommerce
    ? await postgresCommerce.claimTelegramOutbox({
        limit: TELEGRAM_OUTBOX_BATCH_SIZE, nowMs, staleBeforeMs, lockToken,
      })
    : claimSqliteTelegramOutbox(lockToken, nowMs, staleBeforeMs);
  for (const row of rows) {
    try {
      const payload = typeof row.payload === "string" ? JSON.parse(row.payload) : row.payload;
      payload.manager = {
        ...(payload.manager || {}),
        id: row.manager_id || payload.manager?.id || null,
        telegram_chat_id: row.chat_id,
        telegram_thread_id: row.thread_id || "",
      };
      const result = await notifyTelegramOrder(payload);
      const messageId = result?.result?.message_id || result?.message_id || "";
      if (postgresCommerce) {
        await postgresCommerce.markTelegramOutboxSent(row.order_id, row.lock_token, messageId);
      } else {
        markSqliteTelegramOutboxSent(row, messageId);
      }
      ...
      const exponentialDelay = Math.min(
        TELEGRAM_OUTBOX_RETRY_MAX_MS,
        TELEGRAM_OUTBOX_RETRY_BASE_MS * (2 ** Math.max(0, Number(row.attempts) - 1)),
      );
      const nextAttemptAt = Date.now() + Math.max(exponentialDelay, Number(error.retryAfterMs) || 0);
      if (postgresCommerce) {
        await postgresCommerce.markTelegramOutboxRetry(row.order_id, row.lock_token, error.message, nextAttemptAt);
      } else {
        markSqliteTelegramOutboxRetry(row, error, nextAttemptAt);
      }
      console.error("Telegram order notification failed:", error.message);
      audit("system", "telegram.order_failed", {
  ...
  return rows.length >= TELEGRAM_OUTBOX_BATCH_SIZE;
}

function scheduleTelegramOutbox(delayMs = TELEGRAM_OUTBOX_POLL_MS) {
  if (!TELEGRAM_ORDERS_ENABLED || telegramOutboxStopping) return;
  if (telegramOutboxRunning) {
    telegramOutboxWakeRequested = true;
    return;
  }
  ...
  telegramOutboxTimer = setTimeout(() => {
    ...
    telegramOutboxActivePromise = processTelegramOutbox()
      .catch((error) => {
        console.error("Telegram outbox processing failed:", error.message);
        return false;
      })
      .then((moreDue) => {
        telegramOutboxRunning = false;
        const wakeNow = telegramOutboxWakeRequested || moreDue;
        telegramOutboxWakeRequested = false;
        scheduleTelegramOutbox(wakeNow ? 0 : TELEGRAM_OUTBOX_POLL_MS);
      });
  }, Math.max(0, delayMs));
  telegramOutboxTimer.unref();
}

function notifyTelegramOrderLater() {
  scheduleTelegramOutbox(0);
}

async function stopTelegramOutbox() {
  telegramOutboxStopping = true;
  if (telegramOutboxTimer) clearTimeout(telegramOutboxTimer);
  telegramOutboxTimer = null;
  await telegramOutboxActivePromise;
}
...
function catalogOverrideMap() {
  try {
    return new Map(db.prepare("SELECT catalog_id, active, preorder FROM catalog_product_overrides").all().map((r) => [
      Number(r.catalog_id),
      { active: Number(r.active) !== 0, preorder: Number(r.preorder) === 1 },
    ]));
  } catch {
    return new Map();
  }
...
function applyCatalogOverrides(products) {
  ...
  return products.map((p) => {
    const override = overrides.get(Number(p.id));
    if (!override) return p;
    const updated = {
      ...p,
      active: override.active,
      preorder: override.preorder,
      in_stock: !override.preorder,
    };
    return {
      ...updated,
      ...productAvailabilityContract(updated),
    };
  });
}

function setCatalogProductAvailability(id, { active, preorder }) {
  db.prepare(`
    INSERT INTO catalog_product_overrides (catalog_id, active, preorder, updated_at)
    VALUES (?,?,?,datetime('now'))
    ON CONFLICT(catalog_id) DO UPDATE SET
      active=excluded.active,
      preorder=excluded.preorder,
      updated_at=datetime('now')
  `).run(Number(id), active ? 1 : 0, preorder ? 1 : 0);
}
...
/*
 * Authoritative availability contract used by the public API and checkout.
 * A NULL stock value means stock is not tracked (orders are unlimited); zero
 * is an explicitly sold-out value. Product visibility and preorder state take
 * precedence over stock so every catalog backend exposes the same semantics.
 */
function nullableStock(value, precision = 0) {
...
function productAvailabilityContract(product = {}) {
  const active = product.active !== false && Number(product.active) !== 0;
  const preorder = product.preorder === true || Number(product.preorder) === 1;
  const retailEnabled = product.retail_enabled !== false && Number(product.retail_enabled) !== 0;
  const remainingQop = nullableStock(product.available_qop, 3);
  const remainingUnits = nullableStock(product.retail_stock, 0);

  let wholesaleStatus = "in_stock";
  if (!active) wholesaleStatus = "hidden";
  else if (preorder) wholesaleStatus = "preorder";
  else if (remainingQop !== null && remainingQop <= 0) wholesaleStatus = "out_of_stock";

  let retailStatus = "in_stock";
  if (!active) retailStatus = "hidden";
  else if (preorder) retailStatus = "preorder";
  else if (!retailEnabled) retailStatus = "unavailable";
  else if (remainingUnits !== null && remainingUnits <= 0) retailStatus = "out_of_stock";

  const availability_wholesale = {
    status: wholesaleStatus,
    tracked: remainingQop !== null,
    available: wholesaleStatus === "in_stock",
    remaining_qop: remainingQop,
  };
  const availability_retail = {
    status: retailStatus,
    tracked: remainingUnits !== null,
    available: retailStatus === "in_stock",
    remaining_units: remainingUnits,
  };
  return {
    availability_wholesale,
    availability_retail,
    can_order_wholesale: availability_wholesale.available,
    can_order_retail: availability_retail.available,
  };
}

function decorateProduct(p, publicEngagement = null) {
  const summary = publicEngagement || reviewSummary(p.id, p.slug);
  ...
  const reviews = Number(summary.count) || 0;
  const rating = reviews ? Math.round(Number(summary.avg || 0) * 10) / 10 : 0;
  const wholesale = Number(p.wholesale_price || p.price || 0);
  const retail = Number(p.retail_price || p.price || wholesale || 0);
  const colors = productColors(p);
  const availability = productAvailabilityContract(p);
  return {
    ...p,
    price: wholesale,
    colors,
    ...
    /* Visibility and availability are separate. A public product can be
       either in stock or under order; active=false remains genuinely hidden. */
    preorder: Number(p.preorder) === 1,
    /* Legacy field retained for existing clients; mode-specific availability
       fields below are authoritative for new wholesale and retail flows. */
    in_stock: Number(p.preorder) !== 1,
    retail_enabled: p.retail_enabled !== false && Number(p.retail_enabled) !== 0,
    retail_price: retail,
    retail_stock: availability.availability_retail.remaining_units,
    available_qop: availability.availability_wholesale.remaining_qop,
    ...availability,
    like_count: publicEngagement
      ? Math.max(0, Number(publicEngagement.likes) || 0)
      : likeCount(p.id, p.slug),
...
function productSearchText(p) {
  return smartNormalize([
    ...
    p.name_i18n?.en, p.name_i18n?.ru, p.name_i18n?.uz,
...
async function modelVariants(product) {
  const model = String(product?.model_no || "").trim();
  if (!model) return [];
  const rows = postgresCatalog
    ? await postgresCatalog.variantsByModel(model, 24)
    : db.prepare(
      `SELECT id, slug, variant, color, images, active, preorder FROM products
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      color: r.color || "",
      image: images[0] || "",
      in_stock: r.preorder !== true && Number(r.preorder) !== 1,
    };
  });
}
...
function productSeo(product) {
  ...
  const availability = product.availability_wholesale || productAvailabilityContract(product).availability_wholesale;
  const schema = {
  ...
  if (price > 0) {
    schema.offers = {
      ...
      availability: availability.status === "preorder"
...
async function sitemapXml() {
  const staticPaths = ["/", "/shop", "/ordering", "/support", "/terms", "/privacy", "/delete-account"];
...
const PRODUCT_TYPE_COPY = {
  en: {
    ...
    capri: "Capri pants", shorts: "Shorts", top: "Top",
  },
  ru: {
    ...
    capri: "Бриджи", shorts: "Шорты", top: "Майка",
  },
  uz: {
    ...
    capri: "Kapri shim", shorts: "Shortik", top: "Mayka",
  },
};
...
function normalizeProductLocalizedCopy(p) {
  ...
  if (manual) {
    ...
    p.fabric = {
      en: cleanFabricText(p.fabric?.en),
      ru: cleanFabricText(p.fabric?.ru),
      uz: cleanFabricText(p.fabric?.uz),
    };
  } else {
    p.desc = {
...
function rowToProduct(r, lite = false) {
  ...
  const publicEngagement = r.public_review_count === undefined
    ? null
    : {
      count: Number(r.public_review_count) || 0,
      avg: Number(r.public_rating) || 0,
      likes: Number(r.public_like_count) || 0,
    };
  const p = decorateProduct({
    ...
    catalog_panel: referenceAllows("catalog_panel", r.catalog_panel, CATALOG_PANELS) ? r.catalog_panel : inferCatalogPanel(r),
    ...
    retail_stock: r.retail_stock,
    available_qop: r.available_qop,
    like_count: r.like_count || 0,
    sizes: arrayValue(r.sizes), images: arrayValue(r.images),
    tag: r.tag, collection: r.collection || "", size_chart: r.size_chart || "", color: r.color || "", country: r.country || "", material: r.material || "", season: r.season || "", composition: r.composition || "", rating: r.rating, reviews: r.reviews, views: r.views || 0, active: !!r.active, preorder: Number(r.preorder) === 1, sort: r.sort,
    copy_manual: r.copy_manual === true || Number(r.copy_manual) === 1,
  }, publicEngagement);
  if (lite) {
    p.images = p.images.slice(0, 2);
    p.fabric = {
      en: cleanFabricText(r.fabric_en),
      ru: cleanFabricText(r.fabric_ru),
      uz: cleanFabricText(r.fabric_uz),
    };
    return p;
  }
...
function validateProduct(b) {
  ...
  const category = str(b.category, 120).replace(/\s+/g, " ").trim();
  if (!category) throw new Error("category");
  const gender = referenceAllows("gender", b.gender, GENDERS) ? str(b.gender, 80).trim() : "unisex";
  const catalog_panel = referenceAllows("catalog_panel", b.catalog_panel, CATALOG_PANELS)
    ? str(b.catalog_panel, 120).trim()
    : inferCatalogPanel({ ...b, gender });
  const product_type = referenceAllows("product_type", b.product_type, PRODUCT_TYPE_IDS)
    ? str(b.product_type, 120).trim()
    : inferProductType(b);
  ...
  const retail_stock = b.retail_stock === null || b.retail_stock === "" || b.retail_stock === undefined
    ? null
    : Math.max(0, Math.min(1e6, Math.round(Number(b.retail_stock) || 0)));
  ...
  const wholesale_moq = packPieces(sizes, gender, category);
  const images = (Array.isArray(b.images) ? b.images : [])
    ...
    .slice(0, 12);
  const tag = referenceAllows("tag", b.tag, TAGS.filter(Boolean)) ? str(b.tag, 80).trim() : "";
  const collection = referenceAllows("collection", b.collection, COLLECTIONS.filter(Boolean)) ? str(b.collection, 120).trim() : "";
  const rating = Math.min(5, Math.max(0, Number(b.rating) || 0));
  const reviews = Math.min(1e6, Math.max(0, Math.round(Number(b.reviews) || 0)));
  return {
    name, size_chart, color, country, material, season, composition, model_no, variant, gender, category, catalog_panel, product_type, price: wholesale_price, old_price, tag, collection, rating, reviews,
    ...
    fabric_en: cleanFabricText(str(b.fabric?.en, 300)),
    fabric_ru: cleanFabricText(str(b.fabric?.ru, 300)),
    fabric_uz: cleanFabricText(str(b.fabric?.uz, 300)),
    copy_manual: 1,
    active: b.active ? 1 : 0,
...
function validateAvailabilityPatch(body = {}) {
  ...
  const booleanField = (key) => {
    ...
    if (![true, false, 1, 0, "1", "0"].includes(value)) throw new Error(key);
    patch[key] = value === true || value === 1 || value === "1" ? 1 : 0;
  };
  booleanField("active");
  booleanField("preorder");
  booleanField("retail_enabled");
  if (own("available_qop")) {
    const value = body.available_qop;
    if (value === null || value === "") patch.available_qop = null;
    else {
      const parsed = Number(value);
      if (!Number.isFinite(parsed) || parsed < 0 || parsed > 1e6) throw new Error("available_qop");
      patch.available_qop = Math.round(parsed * 1000) / 1000;
    }
  }
  if (own("retail_stock")) {
    const value = body.retail_stock;
    if (value === null || value === "") patch.retail_stock = null;
    else {
      const parsed = Number(value);
      if (!Number.isInteger(parsed) || parsed < 0 || parsed > 1e6) throw new Error("retail_stock");
      patch.retail_stock = parsed;
    }
  }
  if (!Object.keys(patch).length) throw new Error("availability");
  return patch;
}
...
const api = {
  ...
  "PUT /api/auth/profile": async (req, res) => {
    ...
    const phoneChanged = normalizePhone(phone) !== normalizePhone(customer.phone);
    ...
    let updated;
    if (postgresCommerce) {
      updated = await postgresCommerce.updateCustomerProfile(customer.id, {
        name, phone, city, address, phoneChanged, ...business,
      });
    } else {
      db.prepare(`
        UPDATE customers
        SET name=?, phone=?, city=?, address=?, company_name=?, tax_id=?, legal_address=?,
            contact_person=?, expected_volume=?,
            phone_verified=CASE WHEN ? THEN 0 ELSE phone_verified END,
            updated_at=datetime('now')
        WHERE id=?
      `).run(
        name, phone, city, address, business.company_name, business.tax_id,
        business.legal_address, business.contact_person, business.expected_volume,
        phoneChanged ? 1 : 0, customer.id
      );
  ...
  "POST /api/auth/firebase": async (req, res) => {
    ...
    if (payload.email_verified !== true && payload.email_verified !== "true") {
      return fail(res, 401, "firebase_email_not_verified");
    }
    const claimedPhone = str(payload.phone_number || "", 25);
    const providerPhone = /^\+[1-9]\d{7,14}$/.test(claimedPhone) ? claimedPhone : "";
    ...
    const customer = await upsertCustomer({
      ...
      city: str(b.city, 80),
      address: str(b.address, 300),
      phone_verified: providerPhone ? 1 : 0,
  ...
  "GET /api/products": async (req, res, u) => {
    ...
    if (CATALOG_SOURCE_ENABLED) {
      try {
        ...
        const panel = referenceValue("catalog_panel", q.get("panel"), CATALOG_PANELS);
        if (panel) products = products.filter((p) => p.catalog_panel === panel);
        const category = str(q.get("category") || "", 120);
        if (category) products = products.filter((p) => p.category === category);
        const productType = str(q.get("product_type") || "", 40);
        if (referenceAllows("product_type", productType, PRODUCT_TYPE_IDS)) products = products.filter((p) => p.product_type === productType);
        const gender = referenceValue("gender", q.get("gender"), GENDERS);
        if (gender) products = products.filter((p) => p.gender === gender);
        const tag = referenceValue("tag", q.get("tag"), TAGS.filter(Boolean));
        if (tag) products = products.filter((p) => p.tag === tag);
        const collection = referenceValue("collection", q.get("collection"), COLLECTIONS.filter(Boolean));
    ...
    if (postgresCatalog) {
      const result = await postgresCatalog.list({
        activeOnly: true,
        filters: {
          catalog_panel: referenceValue("catalog_panel", q.get("panel"), CATALOG_PANELS),
          category: str(q.get("category") || "", 120),
          product_type: referenceValue("product_type", q.get("product_type"), PRODUCT_TYPE_IDS),
          gender: referenceValue("gender", q.get("gender"), GENDERS),
          tag: referenceValue("tag", q.get("tag"), TAGS.filter(Boolean)),
          collection: referenceValue("collection", q.get("collection"), COLLECTIONS.filter(Boolean)),
        },
    ...
    const args = [];
    const panel = referenceValue("catalog_panel", q.get("panel"), CATALOG_PANELS);
    if (panel) { where += " AND catalog_panel=?"; args.push(panel); }
    const category = str(q.get("category") || "", 120);
    if (category) { where += " AND category=?"; args.push(category); }
    const productType = str(q.get("product_type") || "", 40);
    if (referenceAllows("product_type", productType, PRODUCT_TYPE_IDS)) { where += " AND product_type=?"; args.push(productType); }
    const gender = referenceValue("gender", q.get("gender"), GENDERS);
    if (gender) { where += " AND gender=?"; args.push(gender); }
    const tag = referenceValue("tag", q.get("tag"), TAGS.filter(Boolean));
    if (tag) { where += " AND tag=?"; args.push(tag); }
    const collection = referenceValue("collection", q.get("collection"), COLLECTIONS.filter(Boolean));
    if (collection) { where += " AND collection=?"; args.push(collection); }
    ...
    const sorts = {
      ...
      "popular": `
        (SELECT COUNT(*) FROM reviews AS public_reviews
          WHERE public_reviews.status='approved'
            AND public_reviews.verified_purchase=1
            AND public_reviews.product_id=products.id
            AND public_reviews.product_slug=products.slug) DESC,
        (SELECT COALESCE(AVG(public_ratings.rating),0) FROM reviews AS public_ratings
          WHERE public_ratings.status='approved'
            AND public_ratings.verified_purchase=1
            AND public_ratings.product_id=products.id
            AND public_ratings.product_slug=products.slug) DESC,
        id DESC
      `,
  ...
  "GET /api/search/smart": async (req, res, u) => {
    ...
    let products = await activeProductsForCatalog();
    const gender = referenceValue("gender", u.searchParams.get("gender"), GENDERS);
    const category = u.searchParams.get("category");
    const panel = referenceValue("catalog_panel", u.searchParams.get("panel"), CATALOG_PANELS);
    if (gender) products = products.filter((p) => p.gender === gender);
    if (category) products = products.filter((p) => p.category === category);
    if (panel) products = products.filter((p) => p.catalog_panel === panel);
  ...
  "GET /api/products/:slug": async (req, res, u, m) => {
    const customer = await customerFromRequest(req);
    const identifier = str(m.slug, 120);
    const requestedId = /^\d+$/.test(identifier) ? Number(identifier) : 0;
    const validRequestedId = Number.isSafeInteger(requestedId) && requestedId > 0;
    const row = postgresCatalog
      ? validRequestedId
        ? await postgresCatalog.getById(requestedId, true)
        : await postgresCatalog.getBySlug(identifier, true)
      : validRequestedId
        ? db.prepare(`SELECT * FROM products WHERE id=? AND ${CATALOG_VISIBLE_SQL}`).get(requestedId)
        : db.prepare(`SELECT * FROM products WHERE slug=? AND ${CATALOG_VISIBLE_SQL}`).get(identifier);
    if (row) {
      ...
      return send(res, 200, {
        ...publicProductCard(productForCustomer(localProduct, customer)),
        related,
        variants: await modelVariants(localProduct),
      });
    }
    if (CATALOG_SOURCE_ENABLED) {
      try {
        const product = validRequestedId
          ? await catalogProductById(requestedId)
          : await catalogProductBySlug(identifier);
  ...
  "POST /api/products/:id/view": async (req, res, u, m) => {
    ...
    if (postgresCatalog) {
      const views = await postgresCatalog.incrementViews(id);
      if (views === null) return fail(res, 404, "not_found");
      return send(res, 200, { ok: true, counted: true, views });
    }
  ...
  "POST /api/orders": async (req, res) => {
    ...
    try { idempotency = checkoutIdempotency(req, b, signedInCustomer); }
    catch (error) { return fail(res, 400, error.message); }
    if (idempotency) {
      const existing = postgresCommerce
        ? await postgresCommerce.checkoutIdempotency(idempotency.key)
        : db.prepare(`
        ...
        `).get(idempotency.key);
      if (existing) {
        if (existing.request_hash !== idempotency.requestHash) {
          return fail(res, 409, "idempotency_conflict");
        }
        const response = storedCheckoutResponse(existing);
        if (!response) return fail(res, 409, "idempotency_incomplete");
        return send(res, 201, response, { "Idempotency-Replayed": "true" });
      }
    }
    const c = b.customer || {};
    ...
    if (signedInCustomer && signedInCustomer.approval_status !== "active") {
      return fail(res, 403, "customer_not_active");
    }
    ...
    /*
     * A PostgreSQL catalog cannot be reserved atomically by SQLite commerce.
     * Refuse this deployment mode explicitly instead of decrementing a stale
     * SQLite shadow row (or accepting an untracked item that became tracked).
     */
    if (!checkoutDriversShareTransaction(CATALOG_DB_DRIVER, COMMERCE_DB_DRIVER)) {
      return fail(res, 503, "checkout_database_mismatch");
    }
    let items = [];
    const rawPackMarkup = getSetting("pack_markup");
    const checkoutPackMarkup = rawPackMarkup == null || rawPackMarkup === ""
      ? 0
      : Math.max(0, Math.min(200, Number(rawPackMarkup) || 0));
    const checkoutPricing = customerCheckoutPricing(signedInCustomer);
    let total = 0;
    for (const it of b.items) {
      ...
      let requestedSize = str(it.size || it.selected_size || "", 120);
      const color = requestedColor || productColorOptions[0] || "";
      const availability = productAvailabilityContract(sourceProduct);
      ...
      const modeAvailability = orderType === "retail"
        ? availability.availability_retail
        : availability.availability_wholesale;
      if (modeAvailability.status === "hidden") return fail(res, 400, "item_unavailable");
      if (modeAvailability.status === "preorder") return fail(res, 400, "preorder_unavailable");
      if (modeAvailability.status === "unavailable") return fail(res, 400, "retail_unavailable");
      if (modeAvailability.status === "out_of_stock") return fail(res, 400, "insufficient_stock");
      if (orderType === "wholesale"
        && modeAvailability.tracked
        && wholesalePieces > modeAvailability.remaining_qop * ORDER_BAG_SIZE) {
        return fail(res, 400, "insufficient_stock");
      }
      if (orderType === "retail"
        && modeAvailability.tracked
        && rawQty > modeAvailability.remaining_units) {
        return fail(res, 400, "insufficient_stock");
      }
      if (orderType === "retail") {
        if (!requestedSize) return fail(res, 400, "size_required");
        const availableSizes = (Array.isArray(sizes) ? sizes : [])
          .map((size) => str(String(size), 120))
          .filter(Boolean);
        const matchedSize = availableSizes.find(
          (size) => size.toLocaleLowerCase() === requestedSize.toLocaleLowerCase(),
        );
        if (availableSizes.length && !matchedSize) return fail(res, 400, "invalid_size");
      ...
      if (orderType === "retail") {
        ...
        unit_type = "piece";
        if ((row || postgresCatalog) && modeAvailability.tracked) {
      ...
      items.push({
        ...
        stock_adjustment,
        checkout_guard: checkoutProductSnapshot(sourceProduct, {
          orderType,
          unitType: unit_type,
          packMarkup: checkoutPackMarkup,
        }),
        price_pending,
        price_source: pricing.source,
        price_label: pricing.label,
    ...
    let amount = Math.round(total * 100) / 100;
    const checkoutTestDelay = NODE_ENV === "test"
      ? Math.max(0, Math.min(2_000, Number(process.env.CHECKOUT_TEST_PRETRANSACTION_DELAY_MS) || 0))
      : 0;
    if (checkoutTestDelay) {
      await new Promise((resolve) => setTimeout(resolve, checkoutTestDelay));
    }
    let orderId, paymentId, number, responseBody;
    if (postgresCommerce) {
      let created;
      try {
        created = await postgresCommerce.createCheckout({
          ...
          paymentProvider: paymentProvider(paymentMethod), paymentMethod,
          expectedDiscount: checkoutPricing.discount,
          packMarkup: checkoutPackMarkup,
          orderBagSize: ORDER_BAG_SIZE,
          managerId: manager.id, managerName: manager.name,
          telegramChatId: manager.telegram_chat_id,
          telegramThreadId: manager.telegram_thread_id || "",
          source,
        ...
        if (error.message === "checkout_product_changed") return fail(res, 409, "checkout_product_changed");
        if (error.message === "customer_not_active") return fail(res, 403, "customer_not_active");
        if (error.message === "idempotency_conflict") return fail(res, 409, "idempotency_conflict");
        throw error;
      }
      if (created.replay) {
        return send(res, 201, created.response, { "Idempotency-Replayed": "true" });
      }
      ...
      try {
        if (idempotency) {
          ...
          `).get(idempotency.key);
          if (existing) {
            if (existing.request_hash !== idempotency.requestHash) throw new Error("idempotency_conflict");
            const response = storedCheckoutResponse(existing);
            if (!response) throw new Error("idempotency_incomplete");
            db.exec("COMMIT");
            return send(res, 201, response, { "Idempotency-Replayed": "true" });
          }
        }
        let transactionCustomer = null;
        if (signedInCustomer?.id) {
          transactionCustomer = db.prepare(`
            SELECT approval_status, customer_tier, price_discount
            FROM customers
            WHERE id=?
          `).get(signedInCustomer.id);
        ...
        if (transactionPricing.discount !== checkoutPricing.discount) {
          throw new Error("checkout_product_changed");
        }
        const transactionRawPackMarkup = getSetting("pack_markup");
        const transactionPackMarkup = transactionRawPackMarkup == null || transactionRawPackMarkup === ""
          ? 0
          : Math.max(0, Math.min(200, Number(transactionRawPackMarkup) || 0));
        if (transactionPackMarkup !== checkoutPackMarkup) {
          throw new Error("checkout_product_changed");
        }
        const authoritativeItems = [];
        total = 0;
        for (const draftItem of items) {
          const currentRow = db.prepare("SELECT * FROM products WHERE id=?").get(Number(draftItem.id));
          if (!currentRow) throw new Error("checkout_product_changed");
          const prepared = authoritativeCheckoutLine({
            product: rowToProduct(currentRow),
            draftItem,
            orderType,
            discount: transactionPricing.discount,
            packMarkup: transactionPackMarkup,
            orderBagSize: ORDER_BAG_SIZE,
          });
          if (prepared.stockAdjustment) {
            ...
            const result = adjustment.type === "retail"
              ? db.prepare(`
                UPDATE products
                SET retail_stock=retail_stock-?
                WHERE id=? AND active=1 AND preorder=0 AND retail_enabled=1
                  AND retail_stock IS NOT NULL AND retail_stock>=?
              `).run(adjustment.qty, adjustment.id, adjustment.qty)
              : db.prepare(`
                UPDATE products
                SET available_qop=available_qop-?
                WHERE id=? AND active=1 AND preorder=0
                  AND available_qop IS NOT NULL AND available_qop>=?
        ...
        responseBody = checkoutResponse({ orderId, number, amount, orderType, manager, paymentMethod });
        if (idempotency) {
          db.prepare(`
        ...
        if (error.message === "customer_not_active") return fail(res, 403, "customer_not_active");
        if (error.message === "idempotency_conflict") return fail(res, 409, "idempotency_conflict");
        if (error.message === "idempotency_incomplete") return fail(res, 409, "idempotency_incomplete");
        throw error;
      }
    }
    ...
    send(res, 201, responseBody || checkoutResponse({
      orderId, number, amount, orderType, manager, paymentMethod,
    }));
  },
  ...
  "PUT /api/admin/customers/:id/commercial": async (req, res, u, m) => {
    ...
    const accountType = b.account_type === undefined
      ? (normalizeAccountType(existing.account_type) || "business")
      : normalizeAccountType(b.account_type);
    if (!ACCOUNT_TYPES.includes(accountType)) return fail(res, 400, "account_type");
    const updated = postgresCommerce ? await postgresCommerce.updateCustomerCommercial(id, tier, manager, discount, accountType) :
      (db.prepare(`
      UPDATE customers
      SET customer_tier=?, assigned_manager=?, price_discount=?, account_type=?, updated_at=datetime('now')
      WHERE id=?
    `).run(tier, manager, discount, accountType, id), db.prepare("SELECT * FROM customers WHERE id=?").get(id));
    audit("admin", "customer.commercial_changed", {
      ...
      manager,
      discount,
      from_account_type: existing.account_type || "business",
  ...
  "GET /api/admin/products": async (req, res, u) => {
    ...
    if (postgresCatalog) {
      const result = await postgresCatalog.list({
        filters: { catalog_panel: referenceValue("catalog_panel", q.get("panel"), CATALOG_PANELS) },
    ...
    if (term) {
      ...
      where += ` AND (${searchable.map((field) => `LOWER(${field}) LIKE ?`).join(" OR ")})`;
      args.push(...searchable.map(() => like));
    }
    const total = db.prepare("SELECT COUNT(*) AS c" + where).get(...args).c;
  ...
  "GET /api/admin/catalog-panels": async (req, res) => {
    ...
    const visiblePanels = referenceValues("catalog_panel", CATALOG_PANELS).filter((id) =>
      (CATALOG_PANELS.includes(id) && id !== "clothing_sets")
      || products.some((product) => product.catalog_panel === id)
    );
    send(res, 200, visiblePanels.map((id, index) => {
  ...
  "PATCH /api/admin/products/:id/availability": async (req, res, u, m) => {
    const id = Number(m.id);
    ...
    const b = await readJson(req, 8e3);
    let patch;
    try { patch = validateAvailabilityPatch(b); }
    ...
    if (!existing && CATALOG_SOURCE_ENABLED) {
      ...
      if (Object.keys(patch).some((key) => !["active", "preorder"].includes(key))) {
        return fail(res, 400, "catalog_stock_unavailable");
      }
      const availability = {
        active: Object.hasOwn(patch, "active") ? Boolean(patch.active) : product.active !== false,
        preorder: Object.hasOwn(patch, "preorder") ? Boolean(patch.preorder) : product.preorder === true,
      };
      setCatalogProductAvailability(id, availability);
      const updated = {
        ...product,
        ...availability,
        in_stock: !availability.preorder,
      };
      audit("admin", "catalog_product.availability_changed", { id, fields: Object.keys(patch) });
      return send(res, 200, { ...updated, ...productAvailabilityContract(updated) });
    }
    if (!existing) return fail(res, 404, "not_found");
    ...
    if (postgresCatalog) {
      if (postgresCommerce) await postgresCommerce.ensureFractionalStockSchema();
      updated = await postgresCatalog.update(id, existing.slug, patch);
    } else {
      const fields = Object.keys(patch);
      db.prepare(`UPDATE products SET ${fields.map((field) => `${field}=?`).join(",")} WHERE id=?`)
        .run(...fields.map((field) => patch[field]), id);
      updated = db.prepare("SELECT * FROM products WHERE id=?").get(id);
    }
    audit("admin", "product.availability_changed", { id, fields: Object.keys(patch) });
  ...
  "PUT /api/admin/products/:id": async (req, res, u, m) => {
    ...
    if (!existing && CATALOG_SOURCE_ENABLED) {
      const product = await catalogProductById(id);
      if (!product) return fail(res, 404, "not_found");
      const availability = { active: b.active !== false, preorder: b.preorder === true };
      setCatalogProductAvailability(id, availability);
      audit("admin", "catalog_product.availability_changed", { id, ...availability });
      const updated = (await catalogProductById(id)) || {
        ...product,
        ...availability,
        in_stock: !availability.preorder,
      };
  ...
  "DELETE /api/admin/products/:id": async (req, res, u, m) => {
    ...
    if (!existing && CATALOG_SOURCE_ENABLED) {
      setCatalogProductAvailability(id, { active: false, preorder: false });
  ...
  "POST /api/admin/dictionaries": async (req, res) => {
    const b = await readJson(req, 8e3);
    const kind = str(b.kind, 20);
    const value = str(b.value, 200).replace(/\s+/g, " ").trim();
    if (!DICT_KINDS.includes(kind) || !value) return fail(res, 400, "bad_request");
    const nextSort = Number(db.prepare(
      "SELECT COALESCE(MAX(sort), -1) + 1 next_sort FROM dictionaries WHERE kind=?"
    ).get(kind)?.next_sort || 0);
    db.prepare(
      "INSERT OR IGNORE INTO dictionaries (kind, value, sort) VALUES (?,?,?)"
    ).run(kind, value, nextSort);
    audit("admin", "dictionary.value_added", { kind, value });
    send(res, 201, { ...dictAll(), value });
  },
  ...
  "POST /api/admin/dictionaries/rename": async (req, res) => {
    ...
    if (kind === "sizes") {
      ...
      if (postgresCatalog) {
        touched += await postgresCatalog.renameField(col, from, to);
      }
    }
...
function shutdown(signal) {
  ...
  const outboxStopped = stopTelegramOutbox();
  server.close(async () => {
    try { await outboxStopped; } catch {}
...
async function startServer() {
  ...
  if (postgresCommerce) {
    await postgresCommerce.ensureTelegramOutboxSchema();
...
startServer().catch(async (error) => {
  ...
  try { await stopTelegramOutbox(); } catch {}
  try { await postgresCommerce?.close(); } catch {}
  try { await postgresCatalog?.close(); } catch {}
  try { db.close(); } catch {}
  process.exit(1);
