...
lists Firebase Auth, Cloud Firestore, and Cloud Functions automatic data for
...
| Company name, country, expected purchase volume, sales channels, requested territories, website/social profile, and distributor application message | Yes or potentially identifying | Collected when a user edits a B2B profile or applies | Distributor qualification and sales follow-up; Cloud Firestore, Cloud Functions, and sales operations | Signed-in applications are anonymized during Firebase account deletion; guest lead retention and downstream sales-system deletion require an approved policy |
...
| Product search performed locally | No | Not collected by the current local filter | App functionality; device memory | Not retained after the app session unless included in a submitted chat/support message |
...
- Reconcile automatic Firebase/SDK collection against the final dependency set.
...
Tracking is declared disabled in the iOS application privacy manifest. Google
